...
import { checkOutputDirExit, findDefinitions } from './utils'
...
import { paths, definitions } from './parse'
import { enumFile, modelFile, apiFile } from './generate'

//调试用
function readLocalFile() {
	console.time(chalk.blueBright('耗时'))
	let count = 0
	let filelist = ['../mock/swagger.mock5.json']
	...
	filelist.forEach(url => {
		fs.readFile(path.resolve(__dirname, url), 'utf-8', function (err, data) {
			if (err) {
				...
				setTimeout(() => {
					let res = JSON.parse(data.toString().replace(/\«|\»/g, ''))
...
function compile(swaggerjson, dirname = './') {
	...
	const swaggerDefinitions = swaggerjson.definitions
	//根据Tag创建对应文件夹
	Object.keys(pathMap).forEach(module => {
		...
		const { enumMap, modelMap } = definitions(
			apiConfig.version === 'V2' ? swaggerDefinitions : swaggerjson.components.schemas,
		)
