...
import { getModeleType, transCSharpTypeToTyscriptType, getTagName } from './utils'
...
function getReplaceNames(type) {
	...
	} else if (type.includes('IListResponse')) {
		return {
			name: type.replace('IListResponse', ''),
			toArray: true,
		}
	} else if (type.includes('IResponse')) {
		return {
				...
				if (data.allOf) {
					...
					if (innerData.type === 'array') {
						if (innerData.items.$ref) {
							...
							return `${type}<${transCSharpTypeToTyscriptType(
								innerData.items.type,
								innerData.items.format,
							)}[]>`
						}
					} else {
						if (innerData.$ref) {
...
export function paths(paths) {
	...
	Object.keys(paths).forEach(api => {
		...
		const info = {
			name: new apiConfig.GenerateClass().nameRule
				? new apiConfig.GenerateClass().nameRule(api)
		...
		let tag
		if (data.tags) {
			tag = new apiConfig.GenerateClass().getTagName
				? new apiConfig.GenerateClass().getTagName(api)
				: getTagName(data.tags)
		} else {
