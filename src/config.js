...
function getBaseConfig() {
	return {
		...
		baseUrl: 'https://gateway-test.babycare.com/',
		list: ['/oms/v2/api-docs'],
...
if (!fs.existsSync(path.resolve(process.cwd() + '/auto-port-config.js'))) {
	fs.copyFileSync(
		path.resolve(__dirname, '../src/copyConfig.js'),
		path.resolve(process.cwd() + '/auto-port-config.js'),
	)
	console.log(chalk.greenBright('生成auto-port-config.js文件，请修改baseUrl后再执行一次！！'))
} else {
	const { getUserConfig } = require(path.resolve(process.cwd() + '/auto-port-config.js'))
